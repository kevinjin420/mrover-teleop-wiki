...
You will notice that the ```wypt``` indicator's background has turned yellow. A yellow background indicates a disconnect, and usuually,
...
Looking at your browser window, you should realize that the current layout is not ideal. 

ith the test button and ```ArmControls``` taking up full separate rows. Move the button and the control into the same row by using a div with classes ```d-flex flex-column```, and add spacing as appropriate. 
...
Now find ```ArmControls.vue``` in ```/src/components```. The core functionality in this component has already been implemented. 
